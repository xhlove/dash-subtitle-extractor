...
# from pyshaka.text.TextEngine import TextEngine
from pyshaka.text.VttTextParser import VttTextParser
from pyshaka.util.DataViewReader import DataViewReader, Endianness
# from pyshaka.util.Error import Error
...
class Mp4VttParser:
    ...
    def parseInit(self, data: memoryview):
        ...
        if not self.timescale_:
            raise InvalidMp4VTT('Missing timescale for VTT content. It should be located in the MDHD.')

        if not sawWVTT:
            raise InvalidMp4VTT('A WVTT box should have been seen (a valid vtt init segment with no actual subtitles')
    ...
    def parseMedia(self, data: memoryview, time: TimeContext) -> List[Cue]:
        ...
        def trun_callback(box: ParsedBox):
            ...
            parsedTRUNBox = Mp4BoxParsers.parseTRUN(box.reader, box.version, box.flags)
        ...
        if not sawMDAT and not sawTFDT and not sawTRUN:
            raise InvalidMp4VTT(f'A required box is missing. Is saw: MDAT {sawMDAT} TFDT {sawTFDT} TRUN {sawTRUN}')
        ...
        for presentation in presentations:
            ...
            while True:
                ...
                if presentation.sampleSize and totalSize < presentation.sampleSize:
                    ...
                    break
        assert not reader.hasMoreData(), 'MDAT which contain VTT cues and non-VTT data are not currently supported!'
